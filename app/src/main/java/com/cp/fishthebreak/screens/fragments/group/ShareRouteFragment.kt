...
import com.cp.fishthebreak.adapters.group.ShareRouteAdapter
import com.cp.fishthebreak.databinding.FragmentShareRouteBinding
...
class ShareRouteFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentShareRouteBinding.inflate(layoutInflater, container, false)
        initDataBinding()
    ...
    private fun initDataBinding() {
        binding.lifecycleOwner = this@ShareRouteFragment
        binding.viewModel = viewModel
        val mLayoutManager = LinearLayoutManager(requireContext())
        binding.rv.layoutManager = mLayoutManager
        binding.locationAdapter = ShareRouteAdapter(listOf(), viewModel)
...
//                super.onScrolled(recyclerView, dx, dy)
    ...
    private fun initViewModelResponse() {
        lifecycleScope.launch {
            viewModel.routeResponse.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED)
                .distinctUntilChanged().collect { response ->
                    isPagination = false
                    when (response) {
                        ...
                        is NetworkResult.Error -> {
                            requireActivity().showToast(
                                response.message.getOnErrorMessage(), false
                            )
                            viewModel.resetResponse()
                        }
                        ...
                        is NetworkResult.NoInternet -> {
                            ...
                            requireActivity().showToast(
                                resources.getString(R.string.no_internet), false
                            )
                            viewModel.resetResponse()
                        }
        ...
        lifecycleScope.launch {
            viewModel.deleteResponse.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED)
                .distinctUntilChanged().collect { response ->
                    when (response) {
                        is NetworkResult.Success -> {
                            showHideLoader(false)
        ...
        lifecycleScope.launch {
            viewModel.loadingResponse.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED)
                .distinctUntilChanged().collect { response ->
                    showHideLoader(response)
                }
        }
        lifecycleScope.launch {
            viewModel.notifyAdapter.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED)
                .distinctUntilChanged().collect { response ->
                    if (response) {
                        binding.rv.adapter?.notifyDataSetChanged()
        ...
        lifecycleScope.launch {
            viewModel.navigationResponse.flowWithLifecycle(lifecycle, Lifecycle.State.STARTED)
                .distinctUntilChanged().collect { response ->
                    if (response != null) {
                        when (response) {
                            ...
                            is NavigationDirections.Share -> {
                                groupId?.let { messageId ->
                                    if (response.data is MapUiData.RouteData) {
                                        viewModelSharePoint.shareInGroup(
                                            messageId, "route", response.data.data.id
                                        )
                                    }
                                }
                            }
                            ...
                            is NavigationDirections.ViewPoints -> {
                                if (response.data is MapUiData.RouteData) {
                                    val dialog = SaveRouteBottomSheet(
                                        createListMeasureDistance(response.data.data.points),
    ...
    companion object {
        @JvmStatic
        fun newInstance(groupId: Int) = ShareRouteFragment().apply {
